...
public class ReferenceTreeModel implements TreeModel {
     ...
     * Invoke this method if you've modified the TreeNodes upon which this model
     * depends. The model will notify all of its listeners that the model has
     * changed below the node <code>node</code>.
     */
     ...
     * listenerList.getListenerList(); TreeModelEvent e = null; // Process the
     * listeners last to first, notifying // those that are interested in this
     * event for (int i = listeners.length - 2; i >= 0; i -= 2) { if
     * (listeners[i] == TreeModelListener.class) { // Lazily create the event:
     * if (e == null) e = new TreeModelEvent(source, path); ((TreeModelListener)
