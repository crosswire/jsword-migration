...
public class RowTableModel extends AbstractTableModel {
    /**
     * Builds a RowTable model for the provided (non-null) row list, using the
     * provided row column definition.
     * 
     * @param newList
     *            List
     */
    ...
    /**
     * Method getCellValue Translates from a row index to a row object and asks
     * it for the appropriate cell value
     * 
     * @param rowIndex
     *            int
     * @param columnIndex
    ...
    /**
     * Method removeRow removes a row from the model and causes the display to
    ...
    /**
     * Field indexes provides a look-aside for the sorted view of the table to
     * the row list.
     */
    private int[] indexes;

    /**
     * Field keys provides the primary or composite key of the table. It is a
     * local optimization of columnModel.getSortKeys().
     */
    private int[] keys;

    /**
     * Field sortColumn indicates the column that was last sorted upon. It is
     * initialized the first value in keys, if present otherwise -1
     */
