...
public class WebPage {
     ...
     * 
     * @param browser
     *            The browser that created us, from which we get headers
     * @param url_str
     *            The URL to visit
     */
     ...
     * The page data
     * 
     * public void displayContent() { try { if (mode == 0) { HTMLDocument html =
     * new HTMLDocument(); HTMLEditorKit editor = new HTMLEditorKit();
     * 
     * try { // Shove the page data into a stream // And the read it into a
     * Document editor.read(new StringReader(data), html, 0); } catch (Throwable
     * ex) { if (ex instanceof ThreadDeath) throw (ThreadDeath) ex; // The HTML
     * component regularly throws up without any good reason // ignore him and
     * he will go away. }
     * 
     * // Create the GUI components JFrame frame = new JFrame(url.toString());
     * JTextPane text = new JTextPane(html); JScrollPane scroll = new
     * JScrollPane(text);
     * 
     * // Display them
     ...
     * 
     * try { frame.pack(); } catch (Throwable ex) { if (ex instanceof
     * ThreadDeath) throw (ThreadDeath) ex; // The HTML component regularly
     * throws up without any good reason // ignore him and he will go away. }
     * 
     * frame.setVisible(true); } else { /* HotJavaBrowserBean hjb = new
     * HotJavaBrowserBean(); StringReader sin = new StringReader(data);
     * hjb.setDocumentSource(sin);
     * 
     * final Frame frame = new Frame(url.toString());
     * 
     * // Display them frame.setLayout(new BorderLayout()); frame.add("Center",
     * hjb);
     * 
     * frame.addWindowListener(new WindowAdapter() { public void
     * windowClosing(WindowEvent ev) { frame.setVisible(false); frame.dispose();
     * } });
     * 
     * //frame.pack(); frame.setVisible(true);
     * 
     * } } catch (Throwable ex) { if (ex instanceof ThreadDeath) throw
     * (ThreadDeath) ex; // System.out.println(Level.INFO, "Failure", ex); } }
     */
