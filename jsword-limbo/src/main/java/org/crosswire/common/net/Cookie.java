 ...
 * browser, saved by the browser, and later sent back to the server when the
 * client accesses the same Web page. A cookie's value can uniquely identify a
 ...
 * Some Web browsers have bugs in how they handle the attributes, so use them
 ...
 * each Web server, of at least 4 KB each.
 * 
 * <p>
 * The browser returns cookies to the servlet by adding fields to HTTP request
 * headers. You can retrieve all of the cookies in a request by using the
 * {@link javax.servlet.http.HttpServletRequest#getCookies()} method. Several
 ...
 * 
 * <p>
 * This class both the Version 0 (by Netscape) and Version 1 (by RFC 2109)
...
public class Cookie implements Cloneable {
     ...
     * space or begin with a $ character. You cannot change the cookie's name
     * after you create the cookie.
     * <p>
     * The value can be anything the server chooses to send. Its value is
     * probably of interest only to the server. You can change the cookie's
     * value after the cookie is created with the <code>setValue</code> method.
     * <p>
     * By default, cookies are created according to the Version 0 cookie
     * specification. You can change the version with the
     ...
     * 
     * @param pattern
     *            string containing the domain name within which this cookie is
     ...
     * 
     * @param expiry
     *            an integer specifying the maximum age of the cookie cookie in
     *            seconds; if negative, means the cookie is is not stored; if
     ...
     * <p>
     * If <code>getMaxAge</code> returns a negative value, the cookie was not
     * stored persistently (see {@link #setMaxAge}).
     * <p>
     * This method does not return a zero value, because if a cookie's age was
     * set to zero with <code>setMaxAge</code>, the cookie was deleted.
     * 
    ...
    /**
     * Specifies a path for the cookie, which is the set of URIs (Universal
     * Resource Identifiers, the part of an URL that represents the server path)
     ...
     * include the servlet that set the cookie, for example,
     * <i>servlet/dir1</i>, which makes the cookie visible to all directories on
     * the server under <i>dir1</i>.
     * <p>
     * Consult RFC 2109 (available on the Internet) for more information on
     * setting path names for cookies.
     * 
     * @param uri
     *            a string specifying a path, that contains a servlet name, for
     *            example, <i>servlet/dir1</i>
     * @see #getPath
     */
    ...
    /**
     * Returns the paths (that is, URIs) on the server to which the browser
     * returns this cookie. The cookie is visible to all subdirectories within
     ...
     * secure protocol, such as HTTPS or SSL. You should only use this method
     * when the cookie's originating server used a secure protocol to set the
     * cookie's value.
     * <p>
     ...
     *            only a secure protocol; if false, uses a standard protocol
     * @see #getSecure
     */
     ...
     * secure protocol, or <code>false</code> if the browser can use a standard
     * protocol.
     * 
     * @return <code>true</code> if the browser can use only a standard
    ...
    /**
     * Returns the name of the cookie. You cannot change the name after the
     * cookie is created.
     * 
    ...
    /**
     * Assigns a new value to a cookie after the cookie is created. The value
     * can be anything the server chooses to send, and usually does not make
     * sense to the browser. If you use a binary value, you may want to use
     * BASE64 encoding.
     * <p>
     ...
     * 
     * @param version
     *            0 if the cookie complies with the Netscape spec 1 if the
    ...
    /**
     * Comment=VALUE ... describes cookie's use Discard ... implied by expiry <
     * 0
     */
    ...
    /**
     * Note -- disabled for now to allow full Netscape compatibility from RFC
