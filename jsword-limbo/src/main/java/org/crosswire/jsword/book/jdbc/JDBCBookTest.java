...
public class JDBCBookTest extends BookParentTst {
    ...
    public void testVerseOrdinal() throws Exception {
         ...
         * 
         * for (int b=1; b<=BibleInfo.booksInBible(); b++) { //int max_chapter =
         * BibleInfo.chaptersInBook(b); //int max_verse =
         * BibleInfo.versesInChapter(b, max_chapter);
         * 
         * //test(db.verseOrdinal(b, 1, 1), BibleInfo.verseOrdinal(b, 1, 1));
         * //test(db.verseOrdinal(b, max_chapter, max_verse),
         * BibleInfo.verseOrdinal(b, max_chapter, max_verse));
         * 
         * // Like to do this, but MS can't cope. :-( // for (int c=1;
         * c<=BibleInfo.chaptersIn(b); c++) // for (int v=1;
         * v<=BibleInfo.versesIn(b, c); v++) } }
         */
    }
}
