...
public class LocalRemoter implements Remoter {
     ...
     * 
     * @see
     * org.crosswire.jsword.book.remote.Remoter#execute(org.crosswire.jsword
     * .book.remote.RemoteMethod)
     */
    public Document execute(RemoteMethod method) throws RemoterException {
        ...
        try {
            if (MethodName.GETBIBLES.equals(methodname)) {
                ...
                throw new RemoterException(Msg.REMOTE_NOSUPPORT, new Object[] {
                    methodname
                });
            }
    ...
    /**
     * Create a list of UIDs for the given BibleMetaDatas. This method does rely
     * on the HashMap using equals() to deturmine equality. I'm not sure if it
     * does this.
     * 
     * @param books
     *            The array to create/retrieve UIDs for
    ...
    /**
     * Create a new random UID. This method should never return the same value
     * twice.
     * 
     * @return String
     */
