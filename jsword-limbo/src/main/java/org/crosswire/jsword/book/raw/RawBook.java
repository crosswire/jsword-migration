...
/**
 * RawBook is a custom Bible. It is designed to be:
 * <ul>
 * <li>Compact: So that the download time is as small as possible
 * <li>Divisible: So that a download can be partial, and some text can be read
 * whilst missing content like styles, notes, or even word case.
 * </ul>
 * <p>
 * As a result of this is can be very slow, or very memory hungry. I guess that
 * the technology developed here could be useful as a delivery format, but the
 * timings I am getting from my benchmarks say "start again".
 * </p>
 * 
 * <p>
 * There is a question mark over how this format will handle rich text. The
 * dictionary lookup scheme can be very space efficient but I'm not sure how to
 * embed strongs numbers with the same efficiency.
 * </p>
 * 
 * <p>
 * The algorithm I have implemented here is not perfect. To get a list of the
 * verses it gets 'wrong' see generate.log. There are 2 reasons for problems.
 * The RawBook does not take note of double spaces. And we incorrectly
 * capitalize hyphenated words at the beginning of sentences.
 * </p>
 * 
 * <p>
 * This is in part converted from the VB code that I wrote ages ago that does
 * asimilar job.
 * </p>
 * 
 * <pre>
...
public class RawBook extends AbstractPassageBook {
        ...
        while (sit.hasNext()) {
            ...
            for (Iterator vit = div.getContent().iterator(); vit.hasNext();) {
                ...
                if (data instanceof Element) {
                    ...
                    // Is this verse part of a new paragraph? Since the move to
                    // OSIS
                    // the concept of new para is not what it was. I don't
                    // intend to
                    // fix it properly since Raw does not fit well with
                    // marked-up
                    // text.
     ...
     * 
     * @param words
     *            an array of words to find punctuation from
     ...
     * 
     * @see
     * org.crosswire.jsword.book.local.LocalURLBook#init(org.crosswire.jsword
     * .book.Bible, org.crosswire.jsword.book.WorkListener)
     * 
     * public void generateText(Book source) throws BookException { init(true);
     * 
     ...
     * 
     * public void flush() throws BookException { try { word_items.save();
     * word_insts.save();
     * 
     * punc_items.save(); punc_insts.save();
     * 
     * case_insts.save(); para_insts.save();
     * 
     * // generateSearchCache(); } catch (IOException ex) { throw new
     * BookException(Msg.FLUSH_FAIL, ex); }
     * 
     * super.flush(); }
     */
     ...
     * 
     * private void createSearchCache() throws BookException { try { // Create a
     * passage for each word cache = new Passage[word_items.size()]; for (int
     * i=0; i<word_items.size(); i++) { cache[i] =
     * PassageFactory.createPassage(); }
     * 
     * // For each verse in the Bible for (int ord=1;
     * ord<=BibleInfo.versesInBible(); ord++) { // and each word in the verse
     * int[] word_items = word_insts.getIndexes(ord); for (int i=0;
     * i<word_items.length; i++) { // add the word to that words passage
     * cache[word_items[i]].add(new Verse(ord)); } } } catch
     * (NoSuchVerseException ex) { throw new BookException(Msg.FIND_FAIL, ex); }
     * }
     * 
     * /** Create a cache to speed up searches.
     * 
     * private void deleteSearchCache() throws BookException { cache = null; }
     * 
    ...
    /**
     * Do the Bibles we create cache everything in memory or leave it on disk
     * and then read it at query time.
     * 
     * @param memory
     *            True if we are cacheing data in memory
     */
    ...
    /**
     * Are we cacheing or in on disk mode?. Does this Bible cache everything in
