...
/**
 * Since a Passage no longer implements Collection, a proxy interface might be
 * useful.
 * 
...
public class PassageSet implements SortedSet {
    ...
    public Object[] toArray(Object[] arr) {
        if (ref instanceof RangedPassage) {
            ...
            /*
             * Maybe one day we'll be bothered to make this work if (arr
             * instanceof Verse[]) { try { // The special case for Verse to
             * de-scope VerseRanges // Create a destination array of the correct
             * size: Verse[] retcode = new Verse[countVerses()]; int count = 0;
             * 
             * Enumeration en = rangeElements(); while (en.hasMoreElements()) {
             * // Fill the array with all the Verses VerseRange range =
             * (VerseRange) en.nextElement();
             * 
             * for (int i=0; i<range.getVerseCount(); i++) { retcode[count+1] =
             * new Verse(range.getStart().getOrdinal()+i); } count +=
             * range.getVerseCount(); }
             * 
             * return retcode; } catch (NoSuchVerseException ex) { throw new
             * Error("Logic Error"); } } else { return store.toArray(arr); }
             */
        }
