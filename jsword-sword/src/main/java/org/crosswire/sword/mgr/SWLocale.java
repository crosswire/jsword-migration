...
public class SWLocale {
    ...
    public String translate(String text) {
        ...
        if (entry == null) {
            ...
            if (confEntry == null)
                ...
                lookupTable.put(text, confEntry);
    ...
    /*
     * public Abbrev getBookAbbrevs() { String nullstr = ""; if (bookAbbrevs ==
     * null) { int i; Map bookAbrevs =
     * (Map)localeSource.sections.get("Book Abbrevs"); bookAbbrevs = new struct
     * abbrev[size + 1]; for (i = 0, it =
     * localeSource->Sections["Book Abbrevs"].begin(); it !=
     * localeSource->Sections["Book Abbrevs"].end(); it++, i++) {
     * bookAbbrevs[i].ab = (*it).first.c_str(); bookAbbrevs[i].book =
     * atoi((*it).second.c_str()); } bookAbbrevs[i].ab = nullstr;
     * bookAbbrevs[i].book = -1; }
     * 
     * return bookAbbrevs; }
     * 
     * 
     * public void getBooks(char **iBMAX, struct sbook ***ibooks); void
     * SWLocale::getBooks(char **iBMAX, struct sbook ***ibooks) { if (!BMAX) {
     * BMAX = new char [2]; BMAX[0] = VerseKey::builtin_BMAX[0]; BMAX[1] =
     * VerseKey::builtin_BMAX[1];
     * 
     * books = new struct sbook *[2]; books[0] = new struct sbook[BMAX[0]];
     * books[1] = new struct sbook[BMAX[1]];
     * 
     * for (int i = 0; i < 2; i++) { for (int j = 0; j < BMAX[i]; j++) {
     * books[i][j] = VerseKey::builtin_books[i][j]; books[i][j].name =
     * translate(VerseKey::builtin_books[i][j].name); } } }
     * 
     * iBMAX = BMAX;ibooks = books; }
     */
}
