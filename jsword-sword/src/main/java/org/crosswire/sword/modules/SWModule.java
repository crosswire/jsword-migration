...
public class SWModule {
     ...
     * 
     * ENT: imodname - Internal name for module imoddesc - Name to display to
     * user for module idisp - Display object to use for displaying imodtype -
     * Type of Module (All modules will be displayed with others of same type
     * under their modtype heading
     */
     ...
     * 
     * ENT: imodname - value which to set modname [0] - only get
     * 
     ...
     * 
     * ENT: imoddesc - value which to set moddesc [0] - only get
     * 
     ...
     * 
     * ENT: imodtype - value which to set modtype [0] - only get
     * 
     ...
     * 
     * ENT: idisp - value which to set disp [0] - only get
     * 
     * RET: pointer to disp
     */
    ...
    /*
     * **************************************************************************
     * *** SWModule::Display - Calls this modules display object and passes
     * itself
     * 
     * RET: error status
     */
    ...
    /*
     * **************************************************************************
     * *** SWModule::SetKey - Sets a key to this module for position to a
    ...
    char setKey(SWKey ikey) {
        ...
        if (!ikey.isPersist()) { // if we are to keep our own copy
            ...
            key = ikey; // if we are to just point to an external key
     ...
     * 
     * ENT: ikeytext - value which to set keytext [0] - only get
     * 
     ...
     * RET: *this
     * 
     * void next(int increment) { key.next(increment); error = key.getError(); }
     * 
     * void next() { next(1); }
     */
     ...
     * RET: *this
     * 
     * void previous(int increment) { key.previous(increment); error =
     * key.getError(); }
     */
     ...
     * 
     * ENT: istr - string for which to search searchType - type of search to
     * perform >=0 - regex -1 - phrase -2 - multiword flags - options flags for
     * search
     * 
    ...
    ListKey search(String istr, int searchType, int flags, SWKey scope) {
        ...
        if (searchType == -2) {
            ...
            words = new ArrayList();
            /*
             * words.put(strtok(wordBuf, " "); while (words[wordCount]) {
             * wordCount++; if (wordCount == allocWords) { allocWords+=10; words
     ...
     * 
     * ENT: buf - buf to massage instead of this modules current text len - max
     * len of buf
     * 
     * RET: this module's text at specified key location massaged by Strip
     * filters
     * 
     * String stripText(String buf, int len) { // FilterList::iterator it;
     * 
     * if (buf == null) buf = getText();
     * 
     * for (it = optionfilters.begin(); it != optionfilters.end(); it++) {
     * (*it)->ProcessText(buf, len); } for (it = stripfilters.begin(); it !=
     * stripfilters.end(); it++) { (*it)->ProcessText(buf, len); } return buf; }
     */
     ...
     * RET: listkey set to verses that contain istr
     * 
     * String renderText(String buf, int len) { // FilterList::iterator it;
     * 
     * if (buf == null) buf = getText();
     * 
     * for (it = optionfilters.begin(); it != optionfilters.end(); it++) {
     * (*it)->ProcessText(buf, len); } for (it = renderfilters.begin(); it !=
     * renderfilters.end(); it++) { (*it)->ProcessText(buf, len); } return buf;
     * }
     * 
     * String renderText() { return renderText(null, -1); }
     */
     ...
     * 
     * RET: this module's text at specified key location massaged by
     * RenderFilers
     */
