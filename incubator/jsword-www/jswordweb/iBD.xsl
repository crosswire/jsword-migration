 ...
 * @author DM Smith [dmsmith555 at yahoo dot com]
 ...
 <xsl:stylesheet
  ...
  <!--=======================================================================-->
  <xsl:template match="/">
    <div class="page" xmlns="http://www.w3.org/1999/xhtml" dir="{$direction}">
  ...
  <xsl:template match="verse" mode="print-notes">
    <xsl:if test=".//note[not(@type) or not(@type = 'x-strongsMarkup')]">
      <xsl:variable name="passage" select="@osisID"/>
 ...
 <!-- Avoid adding whitespace -->
  ...
  <xsl:template match="transChange">
    <span class="transChange"><xsl:apply-templates/></span>
  </xsl:template>
  ...
  <xsl:template match="hi" mode="jesus">
      <xsl:choose>
        ...
        <xsl:when test="@type = 'line-through'">
          <span class="strike"><xsl:apply-templates mode="jesus"/></span>
        </xsl:when>
        <xsl:when test="@type = 'normal'">
          <span class="normal"><xsl:apply-templates mode="jesus"/></span>
        </xsl:when>
        <xsl:when test="@type = 'small-caps'">
          <span class="small-caps"><xsl:apply-templates mode="jesus"/></span>
        </xsl:when>
        <xsl:when test="@type = 'sub'">
        ...
        <xsl:when test="@type = 'x-caps'">
          <span class="caps"><xsl:apply-templates mode="jesus"/></span>
        </xsl:when>
        <xsl:otherwise>
          <xsl:apply-templates/>
        </xsl:otherwise>
